...
if datafilelength >0
    ...
    EEG.condition = '';
    EEG.session = [];
%     strfile = cell(1,length(filename));
%     for k = 1:length(filename), strfile{1,k} =['Original file:' pathname filename{1,k}];end
%     EEG.comments = char(strfile);
    ...
    if ~strcmp(strtrim(patientInfo{4}),'X')
        EEG.setname = [EEG.setname ' - ' patientInfo{4} ];
    end
