function [EEG, command] = pop_importNeuracle(foldname, varargin)
...
%   >> [EEG, command] = pop_importNeuracle( foldname );
%   >> [EEG, command] = pop_importNeuracle( foldname, options );
%
% Inputs:
...
%    v1.1: 2019-12-26, add user panel used for selecting channels and blockrange of data
...
if ~isfield(g, 'importevent'), g.importevent = 'on'; end
...
% organize EEG info
% -------------
if length(g.blockrange) == 1,fprintf('Data range should comtain two elements \n ');return,end
if isempty(g.blockrange)
    begsample = 1;
    endsample = sum(datapnts);
else
    begsample = floor(g.blockrange(1)*EEG.srate);
